 ...
 * "commune" => slug de taxonomie commune
...
function query_hades_events($arg=array()) {
    ...
    if ($arg["commune"] && strlen($arg["commune"]) > 1 && !$arg["loc_rayon"]) {
        $tax_queries[] = array(
            'taxonomy' => 'commune', //(string) - Taxonomy.
            'field' => 'slug', //(string) - Select taxonomy term by ('id' or 'slug')
            'terms' => get_query_var("commune"), //(int/string/array) - Taxonomy term(s).
        );
    }		
    ...
    if($arg["commune"] &&  $arg["loc_rayon"]>1) 
    {
